...
select_q <- dbSendQuery(
  conn = con, 
  statement = "SELECT COUNT(*) FROM crimes"
)
select_res <- dbFetch(select_q)
dbClearResult(select_q)
...
Let's return to our old friend, AdventureWorks.  We'll begin by simply pulling sales
order headers from 2012 with the query:
...
soh <- tbl(advw, in_schema('Sales','SalesOrderHeader'))

show_query(soh)

orders_from_2012 <- soh %>%
  select(SalesOrderID, OrderDate, CustomerID, TotalDue) %>%
  filter(year(OrderDate)==2012)

view(orders_from_2012)

show_query(orders_from_2012)

```

Now let’s try the query that pulled the names of employees in the Sales department.  That used the more complex SQL query that you see here in the comments.

I need a few tables to do this:
```{r}
person <- tbl(advw, in_schema('Person', 'Person'))
employee <- tbl(advw, in_schema('HumanResources', 'Employee'))
employeedepartmenthistory <- tbl(advw, in_schema('HumanResources', 'EmployeeDepartmentHistory'))
department <- tbl(advw, in_schema('HumanResources', 'Department'))

#And then I can start working on these with dplyr verbs
...
show_query(sales_employees)

sales_dat <- sales_employees %>% collect()
view(sales_dat)

DBI::dbDisconnect(advw)
```
...
tbl(con, 
    "~/Google Drive/My Drive/Teaching/MSBR70810.Data.Wrangling/data_wrangling/Rail_Equipment_Accident_Incident_Data.csv") |>
  group_by(`Accident Type`) |> # realizing the wisdom of pipes, R 4 natively supports piping! :)
  summarise(mean_killed = mean(`Total Persons Killed`)) |>
  collect()
```
```{r}
system.time(tp <- read_csv("~/Google Drive/My Drive/Teaching/MSBR70810.Data.Wrangling/data_wrangling/Rail_Equipment_Accident_Incident_Data.csv"))
```
