...
factor_vars <- c("cyl", "vs", "am", "gear")

mtcars[, factor_vars] <- lapply(factor_vars, 
                                function(x) as.factor(mtcars[, x]))
...
Let's imagine that we have a set of variables; we can just use the variables from mtcars. 
...
```{r}
predictor_vars <- colnames(mtcars)

predictor_vars <- predictor_vars[-which(predictor_vars == "mpg")] 
...
for(i in 1:predictor_count) {
  ...
  model_formula <- as.formula(paste0("mpg ~ ", predictor))
  
  regression_model <- lm(model_formula, data = mtcars)
...
model_results <- purrr::map_df(.x = predictor_vars, ~{
  model_formula <- as.formula(paste0("mpg ~ ", .x))
